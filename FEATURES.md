...
| 11  | `avg_category_switches`      | Average category switches per session                                     |
...
*In legacy mode, only `num_categories` is used; entropy and gini are excluded.*
...
*In new mode, these are **not** included by default. They are only used in legacy mode.*
...
*Not used for clustering by default; available for post-hoc reporting. Can be enabled via `include_subscriber=True` in `UserFeatureExtractor`.*
